<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="yxdb.aspx.cs" Inherits="GaoKao.ChaXun.yxdb" %>

<!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd">
<html xmlns="http://www.w3.org/1999/xhtml">
<head id="Head1" runat="server">
    <meta http-equiv="Content-Type" content="text/html; charset=utf-8" />
    <title>院校对比查询_格伦高考网</title>
...
<body>
    ...
    <gk:Schoolbanner ID="SchoolBanner1" BannerWord="院校对比" runat="server" />
    <gk:Crumb ID="Crumb1" NavString=" <span>&gt;</span> <a href='/tuijian/default.aspx'>报考智能规划系统</a> <span>&gt;</span> <a href='sjcx.aspx'>高考报考数据查询</a> <span>&gt;</span> 院校对比"
        runat="server" />
    <div class="content" style="background: #fff;">
        <div class="contentbody">
            ...
            <div class="lqdb">
                <ul id="yxdb_tab">
                    <li class="current"><a href="javascript:;">提档分</a></li>
            ...
            $(function () {
                    ...
                    $(".tdinput .fsxyear").change(function () {
                        //获取索引，用来判定，当前操作的是哪一个
                        var indexOftd = $(this).parent().parent().index();//获取当前td的索引
                        //获取每一个td中的下拉框的索引，如果是3，说明当前选项是院校名称，这时候是不需要进行任何操作的      
                    ...
                    $(".duibidx .yichu").click(function () {
                       ...
                       // alert(indexOfyichu);
                        ...
                        list = list.replace(',' + indexOfyichu + ":" + schid + ',', ',');
                        $("#hidList").val(list);
                ...
                function duibiadd(indexOfcxbtn) {
                    ...
                    if (schoolid == 0) {
                        ...
                        //2 将当前id添加到隐藏域
                        var sl=',' + indexOfcxbtn + ":" + schoolid + ',';
                        var list = $("#hidList").val();
                        if (list == "") {
                            list = sl;
                        } else {
                            if (list.indexOf(":" + schoolid + ',') != -1) {
                                alert('已经添加过该院校了');
                                return false;
                            }else {
                                list = list + indexOfcxbtn + ":" + schoolid + ',';
                            }
                        }
                        //3 如果一切正常，此时再获取学校的名称和logo
                        var text = $(".cxtable table tr:first td:eq(" + indexOfcxbtn + ") .fsxyear:eq(3) > option[value='" + schoolid + "']").text();
                        ...
                        _duibidx.attr("s",schoolid);
                        _duibidx.find("a:lt(2)").attr({ "href": "/daxue-jianjie-" + schoolid + ".shtml", "title": "" + text + "" });
                        _duibidx.find("a:eq(1)").text(text);
                        _duibidx.find("img").attr({ "src": "../logo/" + logo + "", "alt": "" + text + "" });

                        //5 显示院校，隐藏筛选条件
                        _duibidx.css("display", "block").siblings().css("display", "none");
                        //6 给当前  td  移除 class='tdinput'
                        _duibidx.parent().removeClass("tdinput");
                        //7 获取当前显示的是哪个分数（0提档分、1平均分，还是 2批次线差），然后调取对应的数据
                        var cxbl=$("#cxlb").val();//0提档分、1平均分、2批次线差
                        getShuJu(cxbl,list);
                        ...
                        <td width="18%" class="tdinput">
                            ...
                            <div class="duibidx" style="display: none;" s="0">
                                <dl>
                                    ...
                                    <dd>
                                        <a target="_blank" href="daxue-jianjie-583.shtml" title="复旦大学">复旦大学</a></dd>
                                    <dd class="yichu">
            ...
            <div class="tjt" id="duibitupian"></div>
        </div>
    </div>
