...
namespace DAL
{
    public class Join_Card
    {
        ...
        public static int Join_CardAdd(Entity.Join_Card info)
        {
            ...
            return Basic.Utils.StrToInt(SqlDB.ExecuteScalar(strCon, CommandType.StoredProcedure, "Join_CardAdd", prams).ToString(), -1);
        }
        ...
        {
            return SqlDB.ExecuteDataset(strCon, CommandType.Text, "SELECT * FROM [Join_Card] WHERE CardId = " + CardId + ";").Tables[0];
        }
        ...
        public static Entity.Join_Card Join_CardEntityGet(int CardId)
        {
            ...
            DataTable dt = SqlDB.ExecuteDataset(strCon, CommandType.Text, "SELECT * FROM [Join_Card] WHERE CardId = " + CardId + ";").Tables[0];
            if (dt.Rows.Count > 0)
            {
        ...
        {
            int intReturnValue = 0;
            intReturnValue = SqlDB.ExecuteNonQuery(strCon, CommandType.Text, "DELETE [Join_Card]  WHERE CardId =  " + CardId);
            if (intReturnValue == 1)
                return true;
            return false;
        }
