...
namespace DAL
{
    ...
    {
        #region  ProfessionalArticle
        /// <summary>
        /// 调用存储过程增加一个
        /// </summary>
        /// <param name="info">实体对象</param>
        /// <returns>正常返回大于 0 的自增id, 0代表重复，否则返回-1</returns>
        public static int ProfessionalArticleAdd(Entity.ProfessionalArticle info)
        {
            SqlParameter[] prams = {
            ...
            return Basic.Utils.StrToInt(SqlDB.ExecuteScalar(CommandType.StoredProcedure, "ProfessionalArticleAdd", prams).ToString(), -1);
        }
        ...
        public static bool ProfessionalArticleEdit(Entity.ProfessionalArticle info)
        {
            SqlParameter[] prams = {
            ...
            intReturnValue = SqlDB.ExecuteNonQuery(CommandType.StoredProcedure, "ProfessionalArticleEdit", prams);
            if (intReturnValue == 1)
                return true;
            return false;
        }
        ...
        public static bool ProfessionalArticlePause(Entity.ProfessionalArticle info)
        {
            int intReturnValue = 0;
            intReturnValue = SqlDB.ExecuteNonQuery(CommandType.Text, "UPDATE [ProfessionalArticle] SET IsPause = " + info.IsPause + "  WHERE Id =  " + info.Id);
            if (intReturnValue == 1)
                return true;
            return false;
        }
        ...
        /// <returns>返回DataTable</returns>
        public static DataTable ProfessionalArticleGet(int Id)
        {
            return SqlDB.ExecuteDataset(CommandType.Text, "SELECT * FROM [ProfessionalArticle] WHERE Id = " + Id + ";").Tables[0];
        }
        ...
        public static Entity.ProfessionalArticle ProfessionalArticleEntityGet(int Id)
        {
            Entity.ProfessionalArticle info = new Entity.ProfessionalArticle();
            DataTable dt = SqlDB.ExecuteDataset(CommandType.Text, "SELECT * FROM [ProfessionalArticle] WHERE Id = " + Id + ";").Tables[0];
            if (dt.Rows.Count > 0)
            {
        ...
        public static bool ProfessionalArticleDel(int Id)
        {
            int intReturnValue = 0;
            intReturnValue = SqlDB.ExecuteNonQuery(CommandType.Text, "DELETE [ProfessionalArticle]  WHERE Id =  " + Id);
            if (intReturnValue == 1)
                return true;
            return false;
        }
        ...
        /// <returns>返回DataTable</returns>
        public static DataTable ProfessionalArticlePageList(string strWhere, int PageSize, int PageIndex)
        {
            StringBuilder sbSql = new StringBuilder();
            sbSql.Append("SELECT * FROM ProfessionalArticle");
            if (!string.IsNullOrEmpty(strWhere.Trim()))
            {
        ...
        public static int ProfessionalArticleCount(string strWhere)
        {
            string strSql;
            if (!string.IsNullOrEmpty(strWhere.Trim()))
                strSql = "SELECT COUNT(*) FROM [ProfessionalArticle] WHERE " + strWhere + ";";
            else
                strSql = "SELECT COUNT(*)  FROM [ProfessionalArticle] ;";
