...
namespace DAL
{
    public class Join_Student
    {
        ...
        public static int Join_StudentAdd(Entity.Join_Student info)
        {
            ...
            return Basic.Utils.StrToInt(SqlDB.ExecuteScalar(strCon, CommandType.StoredProcedure, "Join_StudentAdd", prams).ToString(), -1);
        }
        ...
        {
            return SqlDB.ExecuteDataset(strCon, CommandType.Text, "SELECT * FROM [Join_Student] WHERE StudentId = " + StudentId + ";").Tables[0];
        }
        ...
        {
            int intReturnValue = 0;
            intReturnValue = SqlDB.ExecuteNonQuery(strCon, CommandType.Text, "DELETE [Join_Student]  WHERE StudentId =  " + StudentId);
            if (intReturnValue == 1)
                return true;
            return false;
        }

        public static bool Join_StudentWenLi(int StudentId,int WenLi)
        {
            int intReturnValue = 0;
            intReturnValue = SqlDB.ExecuteNonQuery(strCon, CommandType.Text, "UPDATE [Join_Student] SET WenLi = "+ WenLi +" WHERE StudentId =  " + StudentId);
            if (intReturnValue == 1)
                return true;
            return false;
        }
