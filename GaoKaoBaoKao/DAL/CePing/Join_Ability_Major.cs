...
namespace DAL
{
	public class Join_Ability_Major
    {
		...
		public static bool Join_Ability_MajorEdit(Entity.Join_Ability_Major info)
		{
			...
			intReturnValue = SqlDB.ExecuteNonQuery(strCon,CommandType.StoredProcedure, "Join_Ability_MajorEdit", prams);
			if(intReturnValue == 1)
				return true;
			return false;
		}
        ...
        //{
        //    int intReturnValue = 0;
        //    intReturnValue = SqlDB.ExecuteNonQuery(strCon,CommandType.Text, "UPDATE [Join_Ability_Major] SET IsPause = "+  info.IsPause +"  WHERE MajorId =  " +  info.MajorId);
        //    if(intReturnValue == 1)
        //        return true;
        //    return false;
        //}
		...
		{
			return SqlDB.ExecuteDataset(strCon,CommandType.Text, "SELECT * FROM [Join_Ability_Major] WHERE MajorId = "+MajorId+";").Tables[0];
		}
		...
		public static Entity.Join_Ability_Major Join_Ability_MajorEntityGet(int MajorId)
		{
			...
			DataTable dt = SqlDB.ExecuteDataset(strCon,CommandType.Text, "SELECT * FROM [Join_Ability_Major] WHERE MajorId = "+MajorId+";").Tables[0];
			if(dt.Rows.Count >0)
			{
		...
		{
			int intReturnValue = 0;
			intReturnValue = SqlDB.ExecuteNonQuery(strCon,CommandType.Text, "DELETE [Join_Ability_Major]  WHERE MajorId =  " + MajorId);
			if(intReturnValue == 1)
				return true;
			return false;
		}
