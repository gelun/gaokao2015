...
namespace DAL
{
	public class Join_Results
    {
		...
		public static bool Join_ResultsEdit(Entity.Join_Results info)
		{
			...
			intReturnValue = SqlDB.ExecuteNonQuery(strCon,CommandType.StoredProcedure, "Join_ResultsEdit", prams);
			if(intReturnValue == 1)
				return true;
			return false;
		}
		...
		{
			return SqlDB.ExecuteDataset(strCon,CommandType.Text, "SELECT * FROM [Join_Results] WHERE ResultsId = "+ResultsId+";").Tables[0];
		}
		...
		public static Entity.Join_Results Join_ResultsEntityGet(int ResultsId)
		{
			...
			DataTable dt = SqlDB.ExecuteDataset(strCon,CommandType.Text, "SELECT * FROM [Join_Results] WHERE ResultsId = "+ResultsId+";").Tables[0];
			if(dt.Rows.Count >0)
			{
		...
		{
			int intReturnValue = 0;
			intReturnValue = SqlDB.ExecuteNonQuery(strCon,CommandType.Text, "DELETE [Join_Results]  WHERE ResultsId =  " + ResultsId);
			if(intReturnValue == 1)
				return true;
			return false;
		}
