...
namespace DAL
{
	public class Join_DISCTest
    {
		...
		public static int Join_DISCTestAdd(Entity.Join_DISCTest info)
		{
			...
			return Basic.Utils.StrToInt(SqlDB.ExecuteScalar(strCon,CommandType.StoredProcedure, "Join_DISCTestAdd", prams).ToString(), -1);
		}
		...
		public static bool Join_DISCTestEdit(Entity.Join_DISCTest info)
		{
			...
			intReturnValue = SqlDB.ExecuteNonQuery(strCon,CommandType.StoredProcedure, "Join_DISCTestEdit", prams);
			if(intReturnValue == 1)
				return true;
			return false;
		}
		...
		{
			return SqlDB.ExecuteDataset(strCon,CommandType.Text, "SELECT * FROM [Join_DISCTest] WHERE TestId = "+TestId+";").Tables[0];
		}
		...
		public static Entity.Join_DISCTest Join_DISCTestEntityGet(int TestId)
		{
			...
			DataTable dt = SqlDB.ExecuteDataset(strCon,CommandType.Text, "SELECT * FROM [Join_DISCTest] WHERE TestId = "+TestId+";").Tables[0];
			if(dt.Rows.Count >0)
			{
		...
		{
			int intReturnValue = 0;
			intReturnValue = SqlDB.ExecuteNonQuery(strCon,CommandType.Text, "DELETE [Join_DISCTest]  WHERE TestId =  " + TestId);
			if(intReturnValue == 1)
				return true;
			return false;
		}
