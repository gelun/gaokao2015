...
namespace DAL
{
	public class Join_ZhongKeTest
    {
		...
		public static bool Join_ZhongKeTestEdit(Entity.Join_ZhongKeTest info)
		{
			...
			intReturnValue = SqlDB.ExecuteNonQuery(strCon,CommandType.StoredProcedure, "Join_ZhongKeTestEdit", prams);
			if(intReturnValue == 1)
				return true;
			return false;
		}
		...
		{
			return SqlDB.ExecuteDataset(strCon,CommandType.Text, "SELECT * FROM [Join_ZhongKeTest] WHERE TestId = "+TestId+";").Tables[0];
		}
		...
		public static Entity.Join_ZhongKeTest Join_ZhongKeTestEntityGet(int TestId)
		{
			...
			DataTable dt = SqlDB.ExecuteDataset(strCon,CommandType.Text, "SELECT * FROM [Join_ZhongKeTest] WHERE TestId = "+TestId+";").Tables[0];
			if(dt.Rows.Count >0)
			{
		...
		{
			int intReturnValue = 0;
			intReturnValue = SqlDB.ExecuteNonQuery(strCon,CommandType.Text, "DELETE [Join_ZhongKeTest]  WHERE TestId =  " + TestId);
			if(intReturnValue == 1)
				return true;
			return false;
		}
