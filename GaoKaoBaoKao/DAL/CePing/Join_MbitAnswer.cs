...
namespace DAL
{
	public class Join_MbitAnswer
    {
		...
		public static bool Join_MbitAnswerEdit(Entity.Join_MbitAnswer info)
		{
			...
			intReturnValue = SqlDB.ExecuteNonQuery(strCon,CommandType.StoredProcedure, "Join_MbitAnswerEdit", prams);
			if(intReturnValue == 1)
				return true;
			return false;
		}
		...
		{
			return SqlDB.ExecuteDataset(strCon,CommandType.Text, "SELECT * FROM [Join_MbitAnswer] WHERE AnswerId = "+AnswerId+";").Tables[0];
		}
		...
		public static Entity.Join_MbitAnswer Join_MbitAnswerEntityGet(int AnswerId)
		{
			...
			DataTable dt = SqlDB.ExecuteDataset(strCon,CommandType.Text, "SELECT * FROM [Join_MbitAnswer] WHERE AnswerId = "+AnswerId+";").Tables[0];
			if(dt.Rows.Count >0)
			{
		...
		{
			int intReturnValue = 0;
			intReturnValue = SqlDB.ExecuteNonQuery(strCon,CommandType.Text, "DELETE [Join_MbitAnswer]  WHERE AnswerId =  " + AnswerId);
			if(intReturnValue == 1)
				return true;
			return false;
		}
