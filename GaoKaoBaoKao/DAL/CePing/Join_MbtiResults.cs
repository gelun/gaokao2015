...
namespace DAL
{
	public class Join_MbtiResults
    {
		...
		{
			return SqlDB.ExecuteDataset(strCon,CommandType.Text, "SELECT * FROM [Join_MbtiResults] WHERE Id = "+Id+";").Tables[0];
		}
		...
		public static Entity.Join_MbtiResults Join_MbtiResultsEntityGet(int Id)
		{
			...
			DataTable dt = SqlDB.ExecuteDataset(strCon,CommandType.Text, "SELECT * FROM [Join_MbtiResults] WHERE Id = "+Id+";").Tables[0];
			if(dt.Rows.Count >0)
			{
		...
		{
			int intReturnValue = 0;
			intReturnValue = SqlDB.ExecuteNonQuery(strCon,CommandType.Text, "DELETE [Join_MbtiResults]  WHERE Id =  " + Id);
			if(intReturnValue == 1)
				return true;
			return false;
		}
