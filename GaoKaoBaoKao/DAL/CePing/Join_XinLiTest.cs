...
namespace DAL
{
	public class Join_XinLiTest
    {
		...
		public static int Join_XinLiTestAdd(Entity.Join_XinLiTest info)
		{
			...
			return Basic.Utils.StrToInt(SqlDB.ExecuteScalar(strCon,CommandType.StoredProcedure, "Join_XinLiTestAdd", prams).ToString(), -1);
		}
		...
		public static bool Join_XinLiTestEdit(Entity.Join_XinLiTest info)
		{
			...
			intReturnValue = SqlDB.ExecuteNonQuery(strCon,CommandType.StoredProcedure, "Join_XinLiTestEdit", prams);
			if(intReturnValue == 1)
				return true;
			return false;
		}
		...
		{
			return SqlDB.ExecuteDataset(strCon,CommandType.Text, "SELECT * FROM [Join_XinLiTest] WHERE TestId = "+TestId+";").Tables[0];
		}
		...
		public static Entity.Join_XinLiTest Join_XinLiTestEntityGet(int TestId)
		{
			...
			DataTable dt = SqlDB.ExecuteDataset(strCon,CommandType.Text, "SELECT * FROM [Join_XinLiTest] WHERE TestId = "+TestId+";").Tables[0];
			if(dt.Rows.Count >0)
			{
