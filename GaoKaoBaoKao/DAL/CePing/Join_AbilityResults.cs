...
namespace DAL
{
	public class Join_AbilityResults
    {
		...
		{
			return SqlDB.ExecuteDataset(strCon,CommandType.Text, "SELECT * FROM [Join_AbilityResults] WHERE AbilityResultsId = "+AbilityResultsId+";").Tables[0];
		}
		...
		{
			int intReturnValue = 0;
			intReturnValue = SqlDB.ExecuteNonQuery(strCon,CommandType.Text, "DELETE [Join_AbilityResults]  WHERE AbilityResultsId =  " + AbilityResultsId);
			if(intReturnValue == 1)
				return true;
			return false;
		}
