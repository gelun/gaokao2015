...
namespace DAL
{
	public class Join_ProfessionResults
    {
		...
		{
			return SqlDB.ExecuteDataset(strCon,CommandType.Text, "SELECT * FROM [Join_ProfessionResults] WHERE ProfessionResultsId = "+ProfessionResultsId+";").Tables[0];
		}
		...
		public static Entity.Join_ProfessionResults Join_ProfessionResultsEntityGet(int ProfessionResultsId)
		{
			...
			DataTable dt = SqlDB.ExecuteDataset(strCon,CommandType.Text, "SELECT * FROM [Join_ProfessionResults] WHERE ProfessionResultsId = "+ProfessionResultsId+";").Tables[0];
			if(dt.Rows.Count >0)
			{
		...
		{
			int intReturnValue = 0;
			intReturnValue = SqlDB.ExecuteNonQuery(strCon,CommandType.Text, "DELETE [Join_ProfessionResults]  WHERE ProfessionResultsId =  " + ProfessionResultsId);
			if(intReturnValue == 1)
				return true;
			return false;
		}
