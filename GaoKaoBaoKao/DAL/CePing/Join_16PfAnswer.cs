...
namespace DAL
{
	public class Join_16PfAnswer
    {
		...
		public static int Join_16PfAnswerAdd(Entity.Join_16PfAnswer info)
		{
			...
			return Basic.Utils.StrToInt(SqlDB.ExecuteScalar(strCon,CommandType.StoredProcedure, "Join_16PfAnswerAdd", prams).ToString(), -1);
		}
		...
		public static bool Join_16PfAnswerEdit(Entity.Join_16PfAnswer info)
		{
			...
			intReturnValue = SqlDB.ExecuteNonQuery(strCon,CommandType.StoredProcedure, "Join_16PfAnswerEdit", prams);
			if(intReturnValue == 1)
				return true;
			return false;
		}
		...
		{
			return SqlDB.ExecuteDataset(strCon,CommandType.Text, "SELECT * FROM [Join_16PfAnswer] WHERE AnswerId = "+AnswerId+";").Tables[0];
		}
