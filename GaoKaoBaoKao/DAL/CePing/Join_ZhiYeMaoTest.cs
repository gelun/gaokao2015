...
namespace DAL
{
	public class Join_ZhiYeMaoTest
    {
		...
		public static int Join_ZhiYeMaoTestAdd(Entity.Join_ZhiYeMaoTest info)
		{
			...
			return Basic.Utils.StrToInt(SqlDB.ExecuteScalar(strCon,CommandType.StoredProcedure, "Join_ZhiYeMaoTestAdd", prams).ToString(), -1);
		}
		...
		public static bool Join_ZhiYeMaoTestEdit(Entity.Join_ZhiYeMaoTest info)
		{
			...
			intReturnValue = SqlDB.ExecuteNonQuery(strCon,CommandType.StoredProcedure, "Join_ZhiYeMaoTestEdit", prams);
			if(intReturnValue == 1)
				return true;
			return false;
		}
		...
		{
			return SqlDB.ExecuteDataset(strCon,CommandType.Text, "SELECT * FROM [Join_ZhiYeMaoTest] WHERE TestId = "+TestId+";").Tables[0];
		}
		...
		public static Entity.Join_ZhiYeMaoTest Join_ZhiYeMaoTestEntityGet(int TestId)
		{
			...
			DataTable dt = SqlDB.ExecuteDataset(strCon,CommandType.Text, "SELECT * FROM [Join_ZhiYeMaoTest] WHERE TestId = "+TestId+";").Tables[0];
			if(dt.Rows.Count >0)
			{
		...
		{
			int intReturnValue = 0;
			intReturnValue = SqlDB.ExecuteNonQuery(strCon,CommandType.Text, "DELETE [Join_ZhiYeMaoTest]  WHERE TestId =  " + TestId);
			if(intReturnValue == 1)
				return true;
			return false;
		}
