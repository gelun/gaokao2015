...
namespace DAL
{
	public class Join_JiaoLvTest
    {
		...
		public static int Join_JiaoLvTestAdd(Entity.Join_JiaoLvTest info)
		{
			...
			return Basic.Utils.StrToInt(SqlDB.ExecuteScalar(strCon,CommandType.StoredProcedure, "Join_JiaoLvTestAdd", prams).ToString(), -1);
		}
		...
		public static bool Join_JiaoLvTestEdit(Entity.Join_JiaoLvTest info)
		{
			...
			intReturnValue = SqlDB.ExecuteNonQuery(strCon,CommandType.StoredProcedure, "Join_JiaoLvTestEdit", prams);
			if(intReturnValue == 1)
				return true;
			return false;
		}
		...
		{
			return SqlDB.ExecuteDataset(strCon,CommandType.Text, "SELECT * FROM [Join_JiaoLvTest] WHERE TestId = "+TestId+";").Tables[0];
		}
		...
		public static Entity.Join_JiaoLvTest Join_JiaoLvTestEntityGet(int TestId)
		{
			...
			DataTable dt = SqlDB.ExecuteDataset(strCon,CommandType.Text, "SELECT * FROM [Join_JiaoLvTest] WHERE TestId = "+TestId+";").Tables[0];
			if(dt.Rows.Count >0)
			{
		...
		{
			int intReturnValue = 0;
			intReturnValue = SqlDB.ExecuteNonQuery(strCon,CommandType.Text, "DELETE [Join_JiaoLvTest]  WHERE TestId =  " + TestId);
			if(intReturnValue == 1)
				return true;
			return false;
		}
