...
namespace DAL
{
	public class Join_Test
    {
		...
		public static bool Join_TestEdit(Entity.Join_Test info)
		{
			...
			intReturnValue = SqlDB.ExecuteNonQuery(strCon,CommandType.StoredProcedure, "Join_TestEdit", prams);
			if(intReturnValue == 1)
				return true;
			return false;
		}
		...
		{
            return SqlDB.ExecuteDataset(strCon,CommandType.Text, "SELECT * FROM [Join_Test] WHERE TestId = " + TestId + ";").Tables[0];
		}
        ...
        public static Entity.Join_Test Join_TestEntityGet(int TestId)
		{
            ...
            DataTable dt = SqlDB.ExecuteDataset(strCon,CommandType.Text, "SELECT * FROM [Join_Test] WHERE TestId = " + TestId + ";").Tables[0];
			if(dt.Rows.Count >0)
			{
		...
		{
			int intReturnValue = 0;
            intReturnValue = SqlDB.ExecuteNonQuery(strCon,CommandType.Text, "DELETE [Join_Test]  WHERE TestId =  " + TestId);
			if(intReturnValue == 1)
				return true;
			return false;
		}
