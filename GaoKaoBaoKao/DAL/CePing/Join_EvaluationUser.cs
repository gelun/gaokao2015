...
namespace DAL
{
	public class Join_EvaluationUser
    {
		...
		public static int Join_EvaluationUserAdd(Entity.Join_EvaluationUser info)
		{
			...
			return Basic.Utils.StrToInt(SqlDB.ExecuteScalar(strCon,CommandType.StoredProcedure, "Join_EvaluationUserAdd", prams).ToString(), -1);
		}
		...
		public static bool Join_EvaluationUserEdit(Entity.Join_EvaluationUser info)
		{
			...
			intReturnValue = SqlDB.ExecuteNonQuery(strCon,CommandType.StoredProcedure, "Join_EvaluationUserEdit", prams);
			if(intReturnValue == 1)
				return true;
			return false;
		}
        ...
        //{
        //    int intReturnValue = 0;
        //    intReturnValue = SqlDB.ExecuteNonQuery(strCon,CommandType.Text, "UPDATE [Join_EvaluationUser] SET IsPause = "+  info.IsPause +"  WHERE EvaluationUserId =  " +  info.EvaluationUserId);
        //    if(intReturnValue == 1)
        //        return true;
        //    return false;
        //}
		...
		{
			return SqlDB.ExecuteDataset(strCon,CommandType.Text, "SELECT * FROM [Join_EvaluationUser] WHERE EvaluationUserId = "+EvaluationUserId+";").Tables[0];
		}
		...
		{
			int intReturnValue = 0;
			intReturnValue = SqlDB.ExecuteNonQuery(strCon,CommandType.Text, "DELETE [Join_EvaluationUser]  WHERE EvaluationUserId =  " + EvaluationUserId);
			if(intReturnValue == 1)
				return true;
			return false;
		}
