...
namespace DAL
{
    public class Join_HollandResults
    {
        ...
        public static int Join_HollandResultsAdd(Entity.Join_HollandResults info)
        {
            ...
            return Basic.Utils.StrToInt(SqlDB.ExecuteScalar(strCon, CommandType.StoredProcedure, "Join_HollandResultsAdd", prams).ToString(), -1);
        }
        ...
        public static bool Join_HollandResultsEdit(Entity.Join_HollandResults info)
        {
            ...
            intReturnValue = SqlDB.ExecuteNonQuery(strCon, CommandType.StoredProcedure, "Join_HollandResultsEdit", prams);
            if (intReturnValue == 1)
                return true;
            return false;
        }
        ...
        {
            return SqlDB.ExecuteDataset(strCon, CommandType.Text, "SELECT * FROM [Join_HollandResults] WHERE HollandResultsId = " + HollandResultsId + ";").Tables[0];
        }
        ...
        public static Entity.Join_HollandResults Join_HollandResultsEntityGet(int HollandResultsId)
        {
            ...
            DataTable dt = SqlDB.ExecuteDataset(strCon, CommandType.Text, "SELECT * FROM [Join_HollandResults] WHERE HollandResultsId = " + HollandResultsId + ";").Tables[0];
        ...
        public static Entity.Join_HollandResults Join_HollandResultsEntityGetByUserId(int UserId)
        {
            Entity.Join_HollandResults info = new Entity.Join_HollandResults();
            DataTable dt = SqlDB.ExecuteDataset(strCon, CommandType.Text, "SELECT top 1 * FROM [Join_HollandResults] WHERE UserId = " + UserId + " ORDER BY HollandResultsId DESC").Tables[0];
            if (dt != null && dt.Rows.Count > 0)
            {
                info.HollandResultsId = Basic.Utils.StrToInt(dt.Rows[0]["HollandResultsId"].ToString(), 0);
                info.UserId = Basic.Utils.StrToInt(dt.Rows[0]["UserId"].ToString(), 0);
