...
namespace DAL
{
	public class Join_ProfessionTest
    {
		...
		public static int Join_ProfessionTestAdd(Entity.Join_ProfessionTest info)
		{
			...
			return Basic.Utils.StrToInt(SqlDB.ExecuteScalar(strCon,CommandType.StoredProcedure, "Join_ProfessionTestAdd", prams).ToString(), -1);
		}
		...
		public static bool Join_ProfessionTestEdit(Entity.Join_ProfessionTest info)
		{
			...
			intReturnValue = SqlDB.ExecuteNonQuery(strCon,CommandType.StoredProcedure, "Join_ProfessionTestEdit", prams);
			if(intReturnValue == 1)
				return true;
			return false;
		}
		...
		{
			return SqlDB.ExecuteDataset(strCon,CommandType.Text, "SELECT * FROM [Join_ProfessionTest] WHERE ProfessionId = "+ProfessionId+";").Tables[0];
		}
		...
		public static Entity.Join_ProfessionTest Join_ProfessionTestEntityGet(int ProfessionId)
		{
			...
			DataTable dt = SqlDB.ExecuteDataset(strCon,CommandType.Text, "SELECT * FROM [Join_ProfessionTest] WHERE ProfessionId = "+ProfessionId+";").Tables[0];
			if(dt.Rows.Count >0)
			{
		...
		{
			int intReturnValue = 0;
			intReturnValue = SqlDB.ExecuteNonQuery(strCon,CommandType.Text, "DELETE [Join_ProfessionTest]  WHERE ProfessionId =  " + ProfessionId);
			if(intReturnValue == 1)
				return true;
			return false;
		}
