...
namespace DAL
{
	public class Join_ZhongKeResults
    {
		...
		{
			return SqlDB.ExecuteDataset(strCon,CommandType.Text, "SELECT * FROM [Join_ZhongKeResults] WHERE Id = "+Id+";").Tables[0];
		}
		...
		{
			int intReturnValue = 0;
			intReturnValue = SqlDB.ExecuteNonQuery(strCon,CommandType.Text, "DELETE [Join_ZhongKeResults]  WHERE Id =  " + Id);
			if(intReturnValue == 1)
				return true;
			return false;
		}
