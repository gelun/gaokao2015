...
namespace DAL
{
	public class Join_16PfResults
    {
		...
		public static int Join_16PfResultsAdd(Entity.Join_16PfResults info)
		{
			...
			return Basic.Utils.StrToInt(SqlDB.ExecuteScalar(strCon,CommandType.StoredProcedure, "Join_16PfResultsAdd", prams).ToString(), -1);
		}
		...
		{
			return SqlDB.ExecuteDataset(strCon,CommandType.Text, "SELECT * FROM [Join_16PfResults] WHERE PfResultsId = "+PfResultsId+";").Tables[0];
		}
		...
		public static Entity.Join_16PfResults Join_16PfResultsEntityGet(int PfResultsId)
		{
			...
			DataTable dt = SqlDB.ExecuteDataset(strCon,CommandType.Text, "SELECT * FROM [Join_16PfResults] WHERE PfResultsId = "+PfResultsId+";").Tables[0];
			if(dt.Rows.Count >0)
			{
		...
		{
			int intReturnValue = 0;
			intReturnValue = SqlDB.ExecuteNonQuery(strCon,CommandType.Text, "DELETE [Join_16PfResults]  WHERE PfResultsId =  " + PfResultsId);
			if(intReturnValue == 1)
				return true;
			return false;
		}
