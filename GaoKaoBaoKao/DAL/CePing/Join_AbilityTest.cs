...
namespace DAL
{
	public class Join_AbilityTest
    {
		...
		public static bool Join_AbilityTestEdit(Entity.Join_AbilityTest info)
		{
			...
			intReturnValue = SqlDB.ExecuteNonQuery(strCon,CommandType.StoredProcedure, "Join_AbilityTestEdit", prams);
			if(intReturnValue == 1)
				return true;
			return false;
		}
        ...
        //{
        //    int intReturnValue = 0;
        //    intReturnValue = SqlDB.ExecuteNonQuery(strCon,CommandType.Text, "UPDATE [Join_AbilityTest] SET IsPause = "+  info.IsPause +"  WHERE AbilityId =  " +  info.AbilityId);
        //    if(intReturnValue == 1)
        //        return true;
        //    return false;
        //}
		...
		{
			return SqlDB.ExecuteDataset(strCon,CommandType.Text, "SELECT * FROM [Join_AbilityTest] WHERE AbilityId = "+AbilityId+";").Tables[0];
		}
		...
		public static Entity.Join_AbilityTest Join_AbilityTestEntityGet(int AbilityId)
		{
			...
			DataTable dt = SqlDB.ExecuteDataset(strCon,CommandType.Text, "SELECT * FROM [Join_AbilityTest] WHERE AbilityId = "+AbilityId+";").Tables[0];
			if(dt.Rows.Count >0)
			{
		...
		{
			int intReturnValue = 0;
			intReturnValue = SqlDB.ExecuteNonQuery(strCon,CommandType.Text, "DELETE [Join_AbilityTest]  WHERE AbilityId =  " + AbilityId);
			if(intReturnValue == 1)
				return true;
			return false;
		}
