...
namespace DAL
{
	...
	{
		#region  StudentShouCang
		/// <summary>
		/// 调用存储过程增加一个
		/// </summary>
		/// <param name="info">实体对象</param>
		/// <returns>正常返回大于 0 的自增id, 0代表重复，否则返回-1</returns>
		public static int StudentShouCangAdd(Entity.StudentShouCang info)
		{
			SqlParameter[] prams = {
			...
			return Basic.Utils.StrToInt(SqlDB.ExecuteScalar(CommandType.StoredProcedure, "StudentShouCangAdd", prams).ToString(), -1);
		}
		...
		public static bool StudentShouCangEdit(Entity.StudentShouCang info)
		{
			SqlParameter[] prams = {
			...
			intReturnValue = SqlDB.ExecuteNonQuery(CommandType.StoredProcedure, "StudentShouCangEdit", prams);
			if(intReturnValue == 1)
				return true;
			return false;
		}
		...
		public static DataTable StudentShouCangList(string strWhere)
		{
			string strSql;
			if (!string.IsNullOrEmpty(strWhere.Trim()))
				strSql = "SELECT * FROM [StudentShouCang] WHERE "+ strWhere +";";
			else
				strSql = "SELECT * FROM [StudentShouCang] ;";
		...
		/// <returns>返回DataTable</returns>
		public static DataTable StudentShouCangGet(int Id)
		{
			return SqlDB.ExecuteDataset(CommandType.Text, "SELECT * FROM [StudentShouCang] WHERE Id = "+Id+";").Tables[0];
		}
		...
		/// <returns>返回DataTable</returns>
		public static DataTable StudentShouCangTopGet(string strWhere,int TopNumber)
		{
			string strSql;
			if (!string.IsNullOrEmpty(strWhere.Trim()))
				strSql = "SELECT TOP " + TopNumber.ToString() + " * FROM [StudentShouCang] WHERE "+ strWhere +";";
			else
				strSql = "SELECT TOP " + TopNumber.ToString() + " * FROM [StudentShouCang] ;";
			return SqlDB.ExecuteDataset(CommandType.Text, strSql).Tables[0];
		}
		...
		public static int StudentShouCangCount(string strWhere)
		{
			string strSql;
			if (!string.IsNullOrEmpty(strWhere.Trim()))
				strSql = "SELECT COUNT(*) FROM [StudentShouCang] WHERE "+ strWhere +";";
			else
				strSql = "SELECT COUNT(*)  FROM [StudentShouCang] ;";
