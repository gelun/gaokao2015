...
namespace DAL
{
    public class GaoKaoCard
    {
        ...
        {
            return SqlDB.ExecuteDataset(CommandType.Text, "SELECT * FROM [GaoKaoCard] WHERE ID = " + ID + ";").Tables[0];
        }
        ...
        public static Entity.GaoKaoCard GaoKaoCardEntityGetByStudentId(int StudentId)
        {
            ...
            DataTable dt = SqlDB.ExecuteDataset(CommandType.Text, "SELECT * FROM [GaoKaoCard] WHERE StudentId = " + StudentId + " ORDER BY ID DESC").Tables[0];
            if (dt.Rows.Count > 0)
            {
        ...
        {
            int intReturnValue = 0;
            intReturnValue = SqlDB.ExecuteNonQuery(CommandType.Text, "DELETE [GaoKaoCard]  WHERE ID =  " + ID);
            if (intReturnValue == 1)
                return true;
            return false;
        }
        ...
        public static bool SetGaoKaoCardStudentId(int intStudentId, string KaHao, int ProvinceId)
        {
            ...
            int intReturnValue = 0;
            intReturnValue = SqlDB.ExecuteNonQuery(CommandType.StoredProcedure, "SetGaoKaoCardStudentId", prams);
            if (intReturnValue >= 2)
                return true;
            return false;
        }
        ...
        public static bool GaoKaoCardIsArt(int ID)
        {
            int intReturnValue = 0;
            intReturnValue = SqlDB.ExecuteNonQuery(CommandType.Text, "UPDATE [GaoKaoCard] SET IsArt = 1 WHERE ID =  " + ID);
            if (intReturnValue == 1)
                return true;
            return false;
        }
